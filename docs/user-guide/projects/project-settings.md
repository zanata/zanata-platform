...
To trigger such event, first, the project maintainer will need to add and enable the webhook.
Then the translator can select the language he/she has access to from the languages tab on the version overview page, then from the top right dropdown menu of the documents column, he should be able to fire the event to the registered webhook endpoint.
```json
{
  "project": "zanata",
  "version": "master",
