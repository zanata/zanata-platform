...
@Slf4j
@Transactional
public class LegacyFileMapper implements FileResource {
    ...
    String getDocId(String projectSlug, String iterationSlug,
            String docId, String projectType) {
        String suffix = getSuffix(projectSlug, iterationSlug, projectType, true);
        return StringUtils.removeEnd(docId, suffix);
    }

    private String getSuffix(String projectSlug, String iterationSlug, String projectType, boolean forSourceFiles) {
        // check the database first
        ProjectType projType = getProjectType(projectSlug, iterationSlug);
        if (projType == null) {
            if (isEmpty(projectType)) {
                ...
                // otherwise try the provided projectType
                try {
                    projType = ProjectType.getValueOf(projectType);
                } catch (Exception e) {
                    // getValueOf throws Exception if projectType isn't recognised
                    throw new WebApplicationException("Unknown project type");
                }
            }
        }
        assert projType != null;
    ...
    private @Nullable ProjectType getProjectType(String projectSlug, String iterationSlug) {
        HProjectIteration iter =
                projectIterationDAO.getBySlug(projectSlug, iterationSlug);
        if (iter == null) {
            throw new WebApplicationException("Unknown project or iteration", 404);
        }
        if (iter.getProjectType() != null) {
            return iter.getProjectType();
        }
        HProject proj = projectDAO.getBySlug(projectSlug);
        return proj.getDefaultProjectType();
    }
