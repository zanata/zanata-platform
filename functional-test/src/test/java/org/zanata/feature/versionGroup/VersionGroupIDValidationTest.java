...
public class VersionGroupIDValidationTest
{
   ...
   {
      groupPage = new LoginWorkFlow().signIn("admin", "admin").goToGroups().createNewGroup();
   }
   ...
   public void inputValidationForID(String inputText)
   {
      ...
      // Yes reassign groupPage is necessary since JSF re-renders itself after each field input and selenium is not happy with it
      groupPage = groupPage
            .clearFields()
            .inputGroupId(inputText)
            .inputGroupName(inputText)
            .selectStatus("OBSOLETE")
            .selectStatus("ACTIVE") // this is to avoid ConcurrentModificationException thanks to JSF!!
            .saveGroupFailure();
      assertThat("Validation error is displayed for input:" + inputText, groupPage.getErrors(1), Matchers.contains(errorMsg));
   }
}
