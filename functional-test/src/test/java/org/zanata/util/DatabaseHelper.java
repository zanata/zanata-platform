...
public class DatabaseHelper {
    ...
    private DatabaseHelper() {
        wrapInTryCatch(new Command() {
            ...
            public void execute() throws Exception {
                ...
                connection =
                        DriverManager.getConnection(DatabaseHelper.url,
                                DatabaseHelper.username,
                                DatabaseHelper.password);
                statement = connection.createStatement();
                backupPath =
                        PropertiesHolder.getProperty("zanata.database.backup");
                if (!new File(backupPath).exists()) {
                    // we only want to run this once
                    statement.execute("SCRIPT NOPASSWORDS NOSETTINGS DROP TO '"
                            + backupPath + "' CHARSET 'UTF-8' ");
                }
            }
        });
        wait =
                new FluentWait<Statement>(statement)
                        .ignoring(RuntimeException.class, AssertionError.class)
                        .pollingEvery(1, TimeUnit.SECONDS)
                        .withTimeout(5, TimeUnit.SECONDS)
                        .withMessage("running database reset...");
    }
    ...
    private static void extractConnectionInfo() throws IOException {
        ...
        for (String line : lines) {
            ...
            // in as7 there is no longer a driver class tag. Instead there is a
            // driver tag points to a module.
        ...
        log.info("driver: {}, url: {}, username: {}, password: {}", driver,
                url, username, password);
    }

    private static List<String> readLines(String relativeFilePath)
            throws IOException {
    ...
    private void addUserIfNotExist(final String user) {
        wrapInTryCatch(new Command() {
            ...
            public void execute() throws Exception {
                ...
                ResultSet resultSet =
                        statement
                                .executeQuery("select count(*) from HAccount where username = '"
                                        + user + "'");
                resultSet.next();
    ...
    public void resetDatabaseWithData() {
        String path =
                getFileFromClasspath("org/zanata/feature/zanata_with_data.sql")
                        .getAbsolutePath();
        executeQuery("RUNSCRIPT FROM '" + path + "' CHARSET 'UTF-8'");
        waitUntil("select count(*) from INFORMATION_SCHEMA.tables",
                CURRENT_TABLE_COUNT);
                ...
                wrapInTryCatch(new Command() {
                    ...
                    public void execute() throws Exception {
                        ...
                        assertThat(resultSet.getInt(1),
                                Matchers.equalTo(expectedResultCount));
                    }
                });
                return true;
            }
    ...
    // If we close connection here, in JBoss all the hibernate session won't be
    // usable anymore.
