...
/**
 * This overrides ClassPathSource. It ignores the classpath resource and
...
class GeneratedIndexSource extends ClassPathSource {
    ...
    /**
     * It will generate an index page for a package. Package name will be parsed
     * to sentence as title. Description heading will come from field. If null
     * will use generated title. If test suite class is set, it will use test
     * suite classes to generate an ordered list as links to individual test. If
     * test suite class is not set, it will find all spec files under the
     * package and generate a unordered list of links. Subpackages will generate
     * links according to Concordion breadcrumb rule.
     *
     * @param resource
     *            resource of the package index class. See
     *            http://www.concordion.
     *            org/dist/1.4.2/test-output/concordion/spec
     *            /concordion/results/breadcrumbs/Breadcrumbs.html
    ...
    public InputStream createInputStream(Resource resource) throws IOException {
        ...
        List<Link> specLinks =
                Lists.transform(specFiles, new Function<String, Link>() {
                    @Override
                    public Link apply(String input) {
                        String name = input.substring(0, input.length() - 5);
                        return new Link(input, CaseFormat.UPPER_CAMEL.to(
                                CaseFormat.LOWER_UNDERSCORE, name).replaceAll(
                                "_", " "));
                    }
                });
        ...
        List<Link> allLinks =
                ImmutableList.<Link> builder().addAll(subPackageIndexNames)
                        .addAll(specLinks).build();
        String list =
                createListOfLinksToSpecFiles(allLinks, testSuiteClass != null);

        return new ByteArrayInputStream(String.format(TEMPLATE, title,
                descriptionHeading, list).getBytes("UTF-8"));
    }
    ...
    private static List<Link> getSubPackageIndexNames(Resource resource) {
        ...
        List<File> packages =
                Lists.newArrayList(parentDir.listFiles(new FileFilter() {
                    @Override
    ...
    private List<String> getSpecFilesFromTestSuite(Resource resource) {
        ...
        specFiles.remove(resource.getName()); // we don't want index page itself
                                              // appearing in the list
        return specFiles;
    }

    private static String createListOfLinksToSpecFiles(List<Link> links,
        ...
        List<String> listItems =
                Lists.transform(links, new Function<Link, String>() {
                    @Override
