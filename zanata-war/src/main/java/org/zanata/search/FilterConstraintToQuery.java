...
public class FilterConstraintToQuery
{
   ...
   protected String buildSearchCondition()
   {
      ...
      if (constraints.isSearchInTarget())
      {
         ...
         // @formatter:on
         searchInTargetCondition = QueryBuilder.exists().from("HTextFlowTarget").where(targetWhereClause.toString()).toQueryString();
      }
   ...
   {
      String propertyAlias = Strings.isNullOrEmpty(alias) ? "content" : alias + ".content";
      String caseFunction = constraints.isCaseSensitive() ? "" : "lower";
      Disjunction disjunction = Restrictions.disjunction();
      for (int i = 0; i < 6; i++)
      {
         String contentFieldName = String.format("%s(%s%s)", caseFunction, propertyAlias, i);
   ...
   {
      if (constraints.isIncludeAllState())
      {
         return null;
      }
      Criterion textFlowAndLocaleRestriction = Restrictions.and(eq("textFlow", "tf"), eq("locale", LOCALE_PLACEHOLDER));
      ...
      if (constraints.isIncludeNew())
      {
         ...
         {
            nullTargetCondition = and(nullTargetCondition, contentsCriterion("tf").toString());
         }
