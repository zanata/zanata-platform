      ...
      WorkspaceUsersDisplay.Listener, PublishWorkspaceChatEventHandler
{
   ...
   @Inject
   public WorkspaceUsersPresenter(WorkspaceUsersDisplay display, EventBus eventBus, Identity identity, CachingDispatchAsync dispatcher, WebTransMessages messages, KeyShortcutPresenter keyShortcutPresenter)
   {
   ...
   {
      dispatchChatAction(null, messages.hasJoinedWorkspace(person.getId().toString()), MESSAGE_TYPE.SYSTEM_MSG);
   ...
   public void removeUser(HasManageUserPanel panel, String userId)
   {
      ...
      dispatchChatAction(null, messages.hasQuitWorkspace(userId), MESSAGE_TYPE.SYSTEM_MSG);
   }
}
