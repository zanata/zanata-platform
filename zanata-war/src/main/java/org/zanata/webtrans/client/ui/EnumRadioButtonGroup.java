 ...
 */
public class EnumRadioButtonGroup<E extends Enum<?>> implements ValueChangeHandler<Boolean>
{
   ...
   public EnumRadioButtonGroup(String groupName, Class<E> enumClass, EnumRenderer<E> enumRenderer)
   {
      this.groupName = groupName;
      ImmutableMap.Builder<E, RadioButton> mapBuilder = ImmutableMap.builder();
      ImmutableList.Builder<IsWidget> listBuilder = ImmutableList.builder();
      for (E anEnum : enumClass.getEnumConstants())
      {
         RadioButton radioButton = new RadioButton(groupName, enumRenderer.render(anEnum));
   ...
   public EnumRadioButtonGroup<E> addToContainer(HasWidgets.ForIsWidget container)
   {
      ...
      if (radioButtonChangeEvent.getValue())
      {
         for (Map.Entry<E, RadioButton> entry : radioButtons.entrySet())
         {
            if (entry.getValue() == radioButtonChangeEvent.getSource())
            {
               ...
               {
                  currentSelection = entry.getKey();
                  selectionChangeListener.onSelectionChange(groupName, currentSelection);
               }
            }
         }
      }
