...
@Singleton
public class TranslationHistoryView extends ShortcutContextAwareDialogBox
        implements TranslationHistoryDisplay {
    private static TranslationHistoryViewUiBinder uiBinder = GWT
            .create(TranslationHistoryViewUiBinder.class);
    ...
    private void redrawList() {
        ...
        for (ComparableByDate item : items) {
            if (item instanceof TransHistoryItem) {
                itemList.add(new TransHistoryItemLine((TransHistoryItem) item,
                        listener, stateRenderer));
            }
    ...
    interface TranslationHistoryViewUiBinder extends
            UiBinder<HTMLPanel, TranslationHistoryView> {
    }
