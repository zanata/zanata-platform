...
public class TooltipPopupPanel extends PopupPanel {
    ...
    interface TooltipPopupPanelUiBinder extends
            UiBinder<HTMLPanel, TooltipPopupPanel> {
    }

    private static TooltipPopupPanelUiBinder uiBinder = GWT
            .create(TooltipPopupPanelUiBinder.class);

    @UiField
    Styles style;

    @UiField
    InlineLabel wordTranslated, wordApproved, wordDraft, wordUntranslated,
            wordTotal;

    @UiField
    InlineLabel msgTranslated, msgApproved, msgDraft, msgUntranslated,
            msgTotal;
    @UiField
    Grid grid;
    ...
    private void position(final Element relativeObject, int offsetWidth,
        ...
        if (LocaleInfo.getCurrentLocale().isRTL()) { // RTL case
            ...
            if (offsetWidthDiff > 0) {
                ...
                int textBoxLeftValForRightEdge =
                        textBoxAbsoluteLeft + textBoxOffsetWidth;

                // Distance from the right edge of the text box to the right
                // edge
                // of the window
                int distanceToWindowRight =
                        windowRight - textBoxLeftValForRightEdge;

                // Distance from the right edge of the text box to the left edge
                // of
                // the
                // window
                int distanceFromWindowLeft =
                        textBoxLeftValForRightEdge - windowLeft;
        ...
        // the text box
        int distanceToWindowBottom =
                windowBottom - (top + relativeObject.getOffsetHeight());
