...
public class GetTransMemoryHandler extends
    ...
    public GetTranslationMemoryResult execute(GetTranslationMemory action,
        ...
        HLocale targetLocale =
                localeServiceImpl.getByLocaleId(action.getLocaleId().getId());

        ArrayList<TransMemoryResultItem> results =
                searchTransMemory(targetLocale, transMemoryQuery,
                        action.getSourceLocaleId());
    ...
    protected ArrayList<TransMemoryResultItem> searchTransMemory(
            HLocale targetLocale, TransMemoryQuery transMemoryQuery,
            LocaleId sourceLocaleId) {
        ArrayList<TransMemoryResultItem> results = Lists.newArrayList();
        try {
            List<Object[]> matches;
            matches =
                    translationMemoryQueryService.getSearchResult(
                            transMemoryQuery, sourceLocaleId,
                            targetLocale.getLocaleId(), MAX_RESULTS);
            ...
            for (Object[] match : matches) {
                processIndexMatch(transMemoryQuery, matchesMap, match,
                        sourceLocaleId, targetLocale.getLocaleId());
            }
    ...
    private void processIndexMatch(TransMemoryQuery transMemoryQuery,
        ...
        if (entity instanceof HTextFlowTarget) {
            ...
            ArrayList<String> textFlowContents =
                    new ArrayList<String>(textFlowTarget.getTextFlow()
                            .getContents());
            ArrayList<String> targetContents =
                    new ArrayList<String>(textFlowTarget.getContents());
            ...
            addOrIncrementResultItem(transMemoryQuery, matchesMap, match,
                    MatchType.Imported, sourceContents, targetContents,
                    transUnit.getId(), transUnit.getTranslationMemory()
                            .getSlug());
        }
    }
    ...
    private void addOrIncrementResultItem(TransMemoryQuery transMemoryQuery,
            Map<TMKey, TransMemoryResultItem> matchesMap, Object[] match,
            MatchType matchType, ArrayList<String> sourceContents,
    ...
    private static boolean isValidResult(HTextFlowTarget textFlowTarget) {
        if (textFlowTarget == null || !textFlowTarget.getState().isTranslated()) {
            ...
            HProjectIteration projectIteration =
                    textFlowTarget.getTextFlow().getDocument()
                            .getProjectIteration();
            if (projectIteration.getStatus() == EntityStatus.OBSOLETE
                    || projectIteration.getProject().getStatus() == EntityStatus.OBSOLETE) {
                return false;
            }
        }
        return true;
