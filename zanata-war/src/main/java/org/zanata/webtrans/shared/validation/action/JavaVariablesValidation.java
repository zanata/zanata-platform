...
public class JavaVariablesValidation extends AbstractValidationAction {
    ...
    public List<String> doValidate(String source, String target) {
        ...
        for (Entry<String, Integer> sourceVar : sourceInfo.varCounts.entrySet()) {
            Integer targetCount =
                    targetInfo.varCounts.remove(sourceVar.getKey());
            if (targetCount == null) {
                Integer quotedCount =
                        targetInfo.quotedVarCounts.get(sourceVar.getKey());
        ...
        if (looksLikeMessageFormatString
                && sourceInfo.singleApostrophes != targetInfo.singleApostrophes) {
    ...
    private HashMap<String, Integer> countIndices(ArrayList<String> fullVars) {
        HashMap<String, Integer> argumentIndexCounts =
                new HashMap<String, Integer>();
        for (String fullVar : fullVars) {
            int argIndexEnd = fullVar.indexOf(',');
            argIndexEnd =
                    (argIndexEnd != -1 ? argIndexEnd : fullVar.length() - 1);
            ...
            if (argumentIndexCounts.containsKey(argumentIndex))
                argumentIndexCounts.put(argumentIndex,
                        argumentIndexCounts.get(argumentIndex) + 1);
            else
        ...
        for (int i = 0; i < inString.length(); i++) {
            ...
            if (isQuoted) {
                ...
                if (c == '{') {
                    ...
                    String variable =
                            inString.substring(quotedOpenings
                                    .remove(quotedOpenings.size() - 1), i + 1);
            ...
            if (c == '{') {
                ...
                String variable =
                        inString.substring(
                                openings.remove(openings.size() - 1), i + 1);
