...
public class ViewAllStatusAction implements Serializable {
    ...
    public static class Status implements Comparable<Status>, Serializable {
        ...
        public Status(String locale, String nativeName,
                TranslationStatistics stats, boolean userInLanguageTeam) {
            this.locale = locale;
    ...
    public void validateIteration() {
        if (this.getProjectIteration() == null) {
            throw new EntityNotFoundException(this.iterationSlug,
    ...
    /*
     * This method is needed as a non-cached version of
     * ViewAllStatusAction#getAllStatus that performs less queries, for the
     * Statistic type drop down. It modifies the statsMap variable, which in
     * turn modifies a cached view of the same map.
     */
    public void refreshStatistic() {
        HProjectIteration iteration =
                projectIterationDAO.getBySlug(this.projectSlug,
                        this.iterationSlug);
        ...
        for (HLocale locale : localeList) {
            TranslationStatistics stats =
                    iterationStats.getStats(locale.getLocaleId().getId(),
                            statsOption);
            if (stats == null) {
    ...
    public List<Status> getAllStatus() {
        ...
        for (HLocale locale : localeList) {
            ...
            if (stats == null) {
                ...
                HTextFlowTarget lastTranslatedTarget =
                        localeServiceImpl.getLastTranslated(projectSlug,
                                iterationSlug, locale.getLocaleId());

                if (lastTranslatedTarget != null) {
                    stats.setLastTranslatedBy(lastTranslatedTarget
                            .getLastModifiedBy().getAccount().getUsername());
                    stats.setLastTranslatedDate(lastTranslatedTarget
                            .getLastChanged());
                    stats.setLastTranslated(getLastTranslated(
                            lastTranslatedTarget.getLastChanged(),
                            lastTranslatedTarget.getLastModifiedBy()
            ...
            if (!statsMap.containsKey(locale.getLocaleId())) {
                boolean isMember =
                        authenticatedAccount != null ? personDAO
                                .isUserInLanguageTeamWithRoles(
                                        authenticatedAccount.getPerson(),
                                        locale, null, null, null) : false;

                Status op =
                        new Status(locale.getLocaleId().getId(),
                                locale.retrieveNativeName(), stats, isMember);
    ...
    public String getCopyTransStartTime() {
        CopyTransTaskHandle handle =
                copyTransManager
                        .getCopyTransProcessHandle(getProjectIteration());
        long durationSinceStart =
                (System.currentTimeMillis() - handle.getStartTime());
    ...
    // Define 4 UI strings: completed by you, canceled by you, completed by
    // [username], canceled by [username]
    // OR define UI strings for each of completed/canceled, and another for
    // 'you'
    public String getCopyTransStatusMessage() {
        if (!isCopyTransRunning()) {
            CopyTransTaskHandle recentProcessHandle =
                    copyTransManager
                            .getCopyTransProcessHandle(getProjectIteration());
            ...
            if (recentProcessHandle.getCancelledBy() != null) {
                ...
                // ... by the same user
                if (recentProcessHandle.getCancelledBy().equals(
                        identity.getCredentials().getUsername())) {
                    message.append("you ");
                }
                // .. by another user
                else {
                    message.append(recentProcessHandle.getCancelledBy())
                            .append(" ");
                }

                // when was it done
                message.append(
                        formatTimePeriod(System.currentTimeMillis()
                                - recentProcessHandle.getCancelledTime()))
                        .append(" ago.");
            }
            // completed
            else {
                ...
                // ... by the same user
                if (recentProcessHandle.getTriggeredBy().equals(
                        identity.getCredentials().getUsername())) {
                    message.append("you ");
                }
                // .. by another user
                else {
                    message.append(recentProcessHandle.getTriggeredBy())
                            .append(" ");
                }
                ...
                message.append(
                        formatTimePeriod(System.currentTimeMillis()
                                - recentProcessHandle.getFinishTime())).append(
                        " ago.");
            }
