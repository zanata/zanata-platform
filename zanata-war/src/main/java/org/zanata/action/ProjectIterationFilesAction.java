...
public class ProjectIterationFilesAction implements Serializable {
    ...
    public void initialize() {
        this.iterationDocuments =
                this.documentDAO.getAllByProjectIteration(this.projectSlug,
                        this.iterationSlug);
        ...
        if (this.documentFileUpload.getFileName().endsWith(".pot")) {
            ...
            DocumentType type =
                    translationFileServiceImpl
                            .getDocumentType(documentFileUpload.getFileName());
            if (translationFileServiceImpl.hasAdapterFor(type)) {
                uploadAdapterFile();
            } else {
                FacesMessages.instance().add(Severity.ERROR,
                        "Unrecognized file extension for {0}.",
    ...
    private void showUploadSuccessMessage() {
        FacesMessages.instance()
                .add(Severity.INFO, "Document file {0} uploaded.",
                        documentFileUpload.getFileName());
    }
        ...
        if (docId == null) {
            docId =
                    translationFileServiceImpl.generateDocId(
                            documentFileUpload.getDocumentPath(),
                            documentFileUpload.getFileName());
        }
        ...
        try {
            Resource doc =
                    translationFileServiceImpl.parseUpdatedPotFile(
                            documentFileUpload.getFileContents(), docId,
                            documentFileUpload.getFileName(), useOfflinePo);

            doc.setLang(new LocaleId(documentFileUpload.getSourceLang()));

            // TODO Copy Trans values
            documentServiceImpl.saveDocument(projectSlug, iterationSlug, doc,
    ...
    private void uploadAdapterFile() {
        ...
        try {
            ...
            InputStream fileContents =
                    new DigestInputStream(documentFileUpload.getFileContents(),
                            md);
            tempFile =
            ...
            FacesMessages.instance().add(Severity.ERROR,
                    "Error generating hash for uploaded document {0}.",
                    fileName);
            return;
        }
        ...
        try {
            ...
            // TODO Copy Trans values
            document =
                    documentServiceImpl.saveDocument(projectSlug,
                            iterationSlug, doc, extensions, false);
            showUploadSuccessMessage();
        } catch (SecurityException e) {
            FacesMessages.instance().add(Severity.ERROR,
                    "Error reading uploaded document {0} on server.", fileName);
        } catch (ZanataServiceException e) {
            FacesMessages.instance().add(Severity.ERROR,
                    "Invalid document format for {0}.", fileName);
        }
    ...
    public boolean hasOriginal(String docPath, String docName) {
        GlobalDocumentId id =
                new GlobalDocumentId(projectSlug, iterationSlug, docPath
                        + docName);
    ...
    private Optional<String> getOptionalParams() {
        return Optional.fromNullable(Strings.emptyToNull(documentFileUpload
                .getAdapterParams()));
    }
