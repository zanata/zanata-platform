...
public class SearchResultsPresenterTest {
    ...
    private static final String TEST_MESSAGE_FOCUS_REPLACEMENT_PHRASE_KEY_SHORTCUT =
            "Focus replacement phrase";
    private static final String TEST_MESSAGE_REPLACE_SELECTED_KEY_SHORTCUT =
            "Replace selected text flows";
    private static final String TEST_MESSAGE_SELECT_ALL_TEXT_FLOWS_KEY_SHORTCUT =
            "Select all text flows";
    private static final String TEST_MESSAGE_SHOWING_RESULTS_FOR_SEARCH =
            "Results for search XXX (X text flows in X documents)";
    private static final String TEST_MESSAGE_TOGGLE_ROW_ACTION_BUTTONS =
    ...
    private static final String TEST_MESSAGE_NO_RESULTS_FOR_SEARCH =
            "No results for search XXX";
    ...
    public void beforeMethod() {
        ...
        searchResultsPresenter =
                new SearchResultsPresenter(mockDisplay, mockEventBus,
                        mockDispatcher, mockHistory, mockMessages,
                        mockUserWorkspaceContext, mockKeyShortcutPresenter,
                        mockUndoLinkProvider, mockWindowLocation, contextHolder);

        when(mockMessages.selectAllTextFlowsKeyShortcut()).thenReturn(
                TEST_MESSAGE_SELECT_ALL_TEXT_FLOWS_KEY_SHORTCUT);
        when(mockMessages.focusSearchPhraseKeyShortcut()).thenReturn(
                TEST_MESSAGE_FOCUS_SEARCH_PHRASE_KEY_SHORTCUT);
        when(mockMessages.focusReplacementPhraseKeyShortcut()).thenReturn(
                TEST_MESSAGE_FOCUS_REPLACEMENT_PHRASE_KEY_SHORTCUT);
        when(mockMessages.replaceSelectedKeyShortcut()).thenReturn(
                TEST_MESSAGE_REPLACE_SELECTED_KEY_SHORTCUT);
        when(mockMessages.toggleRowActionButtons()).thenReturn(
        ...
        when(mockDisplay.getReplacementTextBox()).thenReturn(
                mockReplacementTextBox);
        when(mockDisplay.getSelectAllChk()).thenReturn(mockSelectAllChk);
        when(mockDisplay.getRequirePreviewChk()).thenReturn(
                mockRequirePreviewChk);
        when(mockDisplay.getReplaceAllButton())
                .thenReturn(mockReplaceAllButton);

        // getters used after bind
        when(mockDisplay.getCaseSensitiveChk())
                .thenReturn(mockCaseSensitiveChk);
    ...
    public void testExpectedActionsOnBind() {
        ...
        when(mockKeyShortcutPresenter.register(capturedKeyShortcuts.capture()))
        ...
        when(mockUserWorkspaceContext.hasEditTranslationAccess()).thenReturn(
                hasModifyTranslationAccess);

        searchResultsPresenter.bind();

        verify(mockSearchButton).addClickHandler(
        ...
        verify(mockSelectAllChk).addValueChangeHandler(
                capturedSelectAllChkValueChangeHandler.capture());
        verify(mockRequirePreviewChk).addValueChangeHandler(
                capturedRequirePreviewChkValueChangeHandler.capture());
        verify(mockReplaceAllButton).addClickHandler(
                capturedReplaceAllButtonClickHandler.capture());

        verify(mockEventBus).addHandler(eq(TransUnitUpdatedEvent.getType()),
                capturedTransUnitUpdatedEventHandler.capture());
        verify(mockEventBus).addHandler(
                eq(WorkspaceContextUpdateEvent.getType()),
                capturedWorkspaceContextUpdatedEventHandler.capture());

        verify(mockDisplay).setReplaceAllButtonVisible(
                hasModifyTranslationAccess);
    ...
    public void searchButtonClickUpdatesHistory() {
        ...
        expectRunSearch(searchPageHistoryToken(), caseSensitiveFalseValue,
                TEST_SEARCH_PHRASE,
                SearchResultsPresenter.Display.SEARCH_FIELD_TARGET);
    ...
    public void searchWithCaseSensitive() {
        ...
        expectRunSearch(searchPageHistoryToken(), caseSensitiveTrueValue,
                TEST_SEARCH_PHRASE,
                SearchResultsPresenter.Display.SEARCH_FIELD_TARGET);
    ...
    public void searchInSource() {
        expectRunSearch(searchPageHistoryToken(), false, TEST_SEARCH_PHRASE,
                SearchResultsPresenter.Display.SEARCH_FIELD_SOURCE);
    ...
    public void searchInSourceAndTarget() {
        expectRunSearch(searchPageHistoryToken(), false, TEST_SEARCH_PHRASE,
                SearchResultsPresenter.Display.SEARCH_FIELD_BOTH);
    ...
    public void replacementValueChanged() {
        when(mockHistory.getHistoryToken())
                .thenReturn(searchPageHistoryToken());
    ...
    public void firesSearchFromHistoryNoResults() {
        ...
        when(mockMessages.searchForPhraseReturnedNoResults(TEST_SEARCH_PHRASE))
                .thenReturn(TEST_MESSAGE_NO_RESULTS_FOR_SEARCH);
    ...
    public void firesSearchFromHistoryOneResult() {
        ...
        assertThat("text flows for document should be added to data provider",
                dataProviderDoc1List.size(), is(1));
    }
    ...
    private void expectSearchAndDisplaySingleResult() {
        ...
        when(
                mockMessages.showingResultsForProjectWideSearch(
                        TEST_SEARCH_PHRASE, 1, 1)).thenReturn(
        ...
        when(mockDisplay.createMultiSelectionModel()).thenReturn(
                mockSelectionModelDoc1);
        when(
                mockSelectionModelDoc1
                        .addSelectionChangeHandler(capturedSelectionChangeHandlerDoc1
                                .capture())).thenReturn(handlerRegistration);
        when(
                mockSelectionModelDoc1
                        .addSelectionChangeHandler(capturedSelectionChangeDeselectHandlerDoc1
                                .capture())).thenReturn(handlerRegistration);

        when(
                mockDisplay.addDocument(eq(TEST_DOC_PATH_1),
                        capturedViewDocClickHandlers.capture(),
                        capturedSearchDocClickHandlers.capture(),
                        capturedInfoClickHandlers.capture(),
                        capturedSelectionModels.capture(),
                        capturedSelectDocChangeHandlers.capture(),
                        capturedGoToEditorDelegate.capture())).thenReturn(
                mockDataProviderDoc1);

        when(mockDisplay.getSelectAllCheckbox())
                .thenReturn(mockSelectAllHeader);
    }
    ...
    private Answer<GetProjectTransUnitListsResult>
            buildNoSearchResultsResponse() {
        return buildSuccessSearchResponse(new HashMap<Long, String>(),
                new HashMap<Long, List<TransUnit>>());
    }

    private Answer<GetProjectTransUnitListsResult>
            buildSingleTextFlowResponse() {
        final Map<Long, String> docPaths = new HashMap<Long, String>();
        docPaths.put(TEST_DOC_ID_1, TEST_DOC_PATH_1);

        final Map<Long, List<TransUnit>> documents =
                new HashMap<Long, List<TransUnit>>();
        List<TransUnit> docs = new ArrayList<TransUnit>();
        docs.add(TransUnit.Builder.newTransUnitBuilder().setId(TEST_TU_ID_1)
                .setResId(TEST_RES_ID_1).setLocaleId(TEST_LOCALE_ID)
                .setRowIndex(0).setVerNum(TEST_VERNUM_1)
                .addSource(TEST_SOURCE_STRING_1)
                .addTargets(TEST_TARGET_STRING_1).build());
        documents.put(TEST_DOC_ID_1, docs);

        return buildSuccessSearchResponse(docPaths, documents);
    }

    private Answer<GetProjectTransUnitListsResult> buildSuccessSearchResponse(
            final Map<Long, String> docPaths,
            final Map<Long, List<TransUnit>> documents) {
    ...
    private void expectPrepareToDispatchSearch(String searchPhrase,
            boolean caseSensitiveSearch, List<String> queryStringDocuments) {
        ...
        when(mockWindowLocation.getQueryDocuments()).thenReturn(
    ...
    private void expectDispatchSearch(
        ...
        doAnswer(searchResponse).when(mockDispatcher).execute(
                capturedDispatchedSearch.capture(),
                capturedDispatchedSearchCallback.capture());
    }
    ...
    private void expectRunSearch(HistoryToken previousHistoryToken,
            boolean caseSensitive, String searchPhrase, String searchInFields) {
