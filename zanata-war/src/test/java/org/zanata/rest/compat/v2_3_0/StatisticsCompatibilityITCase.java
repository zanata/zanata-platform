...
public class StatisticsCompatibilityITCase extends RestTest
{
   ...
   {
      StatisticsResource statsResource = super.createProxy(createClientProxyFactory(TRANSLATOR, TRANSLATOR_KEY),
            StatisticsResource.class, "/");

      ContainerTranslationStatistics stats = statsResource.getStatistics("sample-project", "1.0", true, true, null);

      int wordStatCount = 0;
      int mssgStatCount = 0;

      assertThat(stats.getStats().size(), greaterThan(0));
      assertThat(stats.getDetailedStats().size(), greaterThan(0)); // Has document stats
      for(TranslationStatistics langStats : stats.getStats())
      {
         ...
         mssgStatCount += (langStats.getUnit() == TranslationStatistics.StatUnit.MESSAGE ? 1 : 0);
      }

      assertThat("Word and Message level stat count should match", wordStatCount == mssgStatCount);
   }
   ...
   public void getStatisticsForDocument() throws Exception
   {
      ...
      ContainerTranslationStatistics stats = statsResource.getStatistics("sample-project", "1.0", "my/path/document.txt", true, null );
   ...
   public void getStatisticsForIterationAndLocale() throws Exception
   {
      ...
      ContainerTranslationStatistics stats = statsResource.getStatistics("sample-project", "1.0", true, false, new String[]{"as"});

      assertThat(stats.getStats().size(), is(1)); // Just one locale and no word level stats
      assertThat(stats.getDetailedStats().size(), greaterThan(0)); // Has document stats
      for(TranslationStatistics langStats : stats.getStats())
      {
         assertThat(langStats.getTotal(),
   ...
   public void getStatisticsForDocumentAndLocale() throws Exception
   {
      ...
      ContainerTranslationStatistics stats = statsResource.getStatistics("sample-project", "1.0", "my/path/document.txt", true, new String[]{"as"} );
