...
public class ExportTransMemoryTest extends TMXStreamingOutputTest {
    ...
    private static void addMetadata(ArrayList<TransMemoryUnit> tuList,
            String srclang) throws TMXParseException {
        for (TransMemoryUnit tu : tuList) {
            ...
            for (TransMemoryUnitVariant tuv : tu.getTransUnitVariants()
                    .values()) {
                Element tuvElem = newTmxElement("tuv");
                tuvElem.addAttribute(new Attribute("xml:lang",
                        XMLConstants.XML_NS_URI, tuv.getLanguage()));
    ...
    private static void addCustomProperty(Element elem, String propType,
            String value) {
