...
public abstract class ResourceTranslationServiceRestTest extends ZanataRestTest {
    ...
    private class MetaTypeAccept implements ClientExecutionInterceptor {
        ...
        public ClientResponse execute(ClientExecutionContext ctx)
            ...
            ctx.getRequest().getHeaders()
                    .add("Content-Type", MediaType.APPLICATION_XML);
            return ctx.proceed();
        }
    ...
    public void setup() throws Exception {
        sourceDocResource =
                getClientRequestFactory().createProxy(
                        ...
                        createBaseURI("/projects/p/" + projectSlug
                                + "/iterations/i/" + iter + "/r"));
        translationResource =
                getClientRequestFactory().createProxy(
    ...
    protected void prepareDBUnitOperations() {
        beforeTestOperations.add(new DataSetOperation(ACCOUNT_DATA_DBUNIT_XML,
                DatabaseOperation.CLEAN_INSERT));
        beforeTestOperations.add(new DataSetOperation(
                DOCUMENTS_DATA_DBUNIT_XML, DatabaseOperation.CLEAN_INSERT));
        beforeTestOperations.add(new DataSetOperation(PROJECTS_DATA_DBUNIT_XML,
                DatabaseOperation.CLEAN_INSERT));
        beforeTestOperations.add(new DataSetOperation(LOCALE_DATA_DBUNIT_XML,
                DatabaseOperation.CLEAN_INSERT));

        afterTestOperations.add(new DataSetOperation(PROJECTS_DATA_DBUNIT_XML,
                DatabaseOperation.DELETE_ALL));
        afterTestOperations.add(new DataSetOperation(DOCUMENTS_DATA_DBUNIT_XML,
                DatabaseOperation.DELETE_ALL));
        afterTestOperations.add(new DataSetOperation(ACCOUNT_DATA_DBUNIT_XML,
                DatabaseOperation.DELETE_ALL));
        afterTestOperations.add(new DataSetOperation(LOCALE_DATA_DBUNIT_XML,
                DatabaseOperation.DELETE_ALL));
    }
}
