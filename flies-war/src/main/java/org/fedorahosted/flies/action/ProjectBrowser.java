...
public class ProjectBrowser
{
   ...
   {
      latestProjects = session.createCriteria(HProject.class).addOrder(Order.asc(ORDERBY_NAME)).setMaxResults(DEFAULT_LIMIT).setComment("ProjectBrowser.getLatestProjects()").list();
   }
   ...
   public void getProjects()
   {
      ...
      projects = session.createCriteria(HProject.class).addOrder(Order.asc(order)).setMaxResults(DEFAULT_LIMIT).setFirstResult((pageNumber - 1) * DEFAULT_LIMIT).setComment("ProjectBrowser.getProjects()").list();
   }

   public Integer getSize()
   {
      return session.createCriteria(HProject.class).setComment("ProjectBrowser.getSize()").list().size();
   }
