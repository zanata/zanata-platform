...
public class ElementBuilderTest
{
   ...
   public void buildElementXMLStreamReader() throws Exception
   {
      ...
      while (reader.hasNext())
      {
         ...
         if (eventType == START_ELEMENT && reader.getLocalName().equals("tu"))
         {
   ...
   public void buildElementStartElementXMLEventReader() throws Exception
   {
      ...
      while (reader.hasNext())
      {
         ...
         if (event.isStartElement() && event.asStartElement().getName().getLocalPart().equals("tu"))
         {
            ElementBuilder.buildElement(event.asStartElement(), reader).toXML();
         }
      }
   }
   ...
   {
      // Nasty way to ensure that we get a Transformer which supports StAXSource:
